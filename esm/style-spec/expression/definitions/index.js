...
CompoundExpression.register(expressions, {
  ...
  'to-string': [StringType, [ValueType], (ctx, [v]) => {
    v = v.evaluate(ctx);
    const type = typeof v;
  ...
  'round': [NumberType, [NumberType], (ctx, [n]) => {
    const v = n.evaluate(ctx); // Javascript's Math.round() rounds towards +Infinity for halfway
  ...
  'filter-in-small': [BooleanType, [StringType, array(ValueType)], // assumes v is an array literal
  (ctx, [k, v]) => v.value.indexOf(ctx.properties()[k.value]) >= 0],
  'filter-in-large': [BooleanType, [StringType, array(ValueType)], // assumes v is a array literal with values sorted in ascending order and of a single type
  (ctx, [k, v]) => binarySearch(ctx.properties()[k.value], v.value, 0, v.value.length - 1)],
  '>': {
    type: BooleanType,
    overloads: [[[NumberType, NumberType], gt], [[StringType, StringType], gt], [[StringType, StringType, CollatorType], gtCollate]]
  },
  '<': {
    type: BooleanType,
    overloads: [[[NumberType, NumberType], lt], [[StringType, StringType], lt], [[StringType, StringType, CollatorType], ltCollate]]
  },
  '>=': {
    type: BooleanType,
    overloads: [[[NumberType, NumberType], gteq], [[StringType, StringType], gteq], [[StringType, StringType, CollatorType], gteqCollate]]
  },
  '<=': {
    type: BooleanType,
    overloads: [[[NumberType, NumberType], lteq], [[StringType, StringType], lteq], [[StringType, StringType, CollatorType], lteqCollate]]
  },
  'all': {
    type: BooleanType,
