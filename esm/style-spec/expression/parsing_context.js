    ...
    if (Array.isArray(expr)) {
      ...
      if (Expr) {
        ...
        if (this.expectedType) {
          ...
          const actual = parsed.type; // When we expect a number, string, boolean, or array but
...
function isConstant(expression) {
  ...
  const isTypeAnnotation = expression instanceof Coercion || expression instanceof Assertion || expression instanceof ArrayAssertion;
  ...
  return isFeatureConstant(expression) && isGlobalPropertyConstant(expression, ['zoom', 'heatmap-density', 'line-progress']);
}