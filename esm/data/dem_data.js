...
register('Level', Level); // DEMData is a data structure for decoding, backfilling, and storing elevation data for processing in the hillshade shaders
...
export default class DEMData {
  constructor(uid, scale, data) {
    this.uid = uid;
    this.scale = scale || 1; // if no data is provided, use a temporary empty level to satisfy flow
  ...
  loadFromImage(data, encoding) {
    ...
    if (encoding && encoding !== "mapbox" && encoding !== "terrarium") return warnOnce(`"${encoding}" is not a valid encoding type. Valid types include "mapbox" and "terrarium".`); // Build level 0

    const level = this.level = new Level(data.width, data.width / 2);
    const pixels = data.data;

    this._unpackData(level, pixels, encoding || "mapbox"); // in order to avoid flashing seams between tiles, here we are initially populating a 1px border of pixels around the image
    ...
    for (let x = 0; x < level.dim; x++) {
      ...
      level.set(-1, x, level.get(0, x)); // right vertical border

      level.set(level.dim, x, level.get(level.dim - 1, x)); // left horizontal border

      level.set(x, -1, level.get(x, 0)); // right horizontal border
  ...
  _unpackData(level, pixels, encoding) {
    const unpackFunctions = {
      "mapbox": this._unpackMapbox,
      "terrarium": this._unpackTerrarium
    };
