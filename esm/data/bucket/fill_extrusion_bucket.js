...
function addVertex(vertexArray, x, y, nx, ny, nz, t, e) {
  ...
  x, y, // a_normal_ed: 3-component normal and 1-component edgedistance
  Math.floor(nx * FACTOR) * 2 + t, ny * FACTOR * 2, nz * FACTOR * 2, // edgedistance (used for wrapping patterns around extrusion sides)
  Math.round(e));
}
  ...
  addFeature(feature, geometry) {
    for (const polygon of classifyRings(geometry, EARCUT_MAX_RINGS)) {
      ...
      for (let j = 0; j < indices.length; j += 3) {
        this.indexArray.emplaceBack(triangleIndex + indices[j], triangleIndex + indices[j + 1], triangleIndex + indices[j + 2]);
      }
