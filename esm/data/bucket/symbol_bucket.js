...
function addVertex(array, anchorX, anchorY, ox, oy, tx, ty, sizeVertex) {
  ...
  anchorX, anchorY, Math.round(ox * 32), Math.round(oy * 32), // a_data
...
class SymbolBuffers {
  ...
  upload(context, dynamicIndexBuffer) {
    this.layoutVertexBuffer = context.createVertexBuffer(this.layoutVertexArray, symbolLayoutAttributes.members);
    this.indexBuffer = context.createIndexBuffer(this.indexArray, dynamicIndexBuffer);
    this.programConfigurations.upload(context);
    this.dynamicLayoutVertexBuffer = context.createVertexBuffer(this.dynamicLayoutVertexArray, dynamicLayoutAttributes.members, true);
    this.opacityVertexBuffer = context.createVertexBuffer(this.opacityVertexArray, shaderOpacityAttributes, true); // This is a performance hack so that we can write to opacityVertexArray with uint32s
...
class SymbolBucket {
  ...
  populate(features, options) {
    ...
    const hasText = (textField.value.kind !== 'constant' || textField.value.value.length > 0) && (textFont.value.kind !== 'constant' || textFont.value.value.length > 0);
  ...
  _addCollisionDebugVertex(layoutVertexArray, collisionVertexArray, point, anchor, extrude) {
    ...
    return layoutVertexArray.emplaceBack( // pos
    point.x, point.y, // a_anchor_pos
    anchor.x, anchor.y, // extrude
    ...
    for (const symbolInstance of this.symbolInstances) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        for (let b = feature.boxStartIndex; b < feature.boxEndIndex; b++) {
          ...
          const y1 = box.y1;
          const x2 = box.x2;
          const y2 = box.y2; // If the radius > 0, this collision box is actually a circle
  ...
  sortFeatures(angle) {
    ...
    this.sortedAngle = angle; // The current approach to sorting doesn't sort across segments so don't try.
    // Sorting within segments separately seemed not to be worth the complexity.

    if (this.text.segments.get().length > 1 || this.icon.segments.get().length > 1) return; // If the symbols are allowed to overlap sort them by their vertical screen position.
