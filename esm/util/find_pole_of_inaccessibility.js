...
export default function (polygonRings, precision = 1, debug = false) {
  ...
  let h = cellSize / 2; // a priority queue of cells in order of their "potential" (max distance to polygon)

  const cellQueue = new Queue(null, compareMax);
  if (cellSize === 0) return new Point(minX, minY); // cover polygon with initial cells
  ...
  while (cellQueue.length) {
    ...
    const cell = cellQueue.pop(); // update the best cell if we found a better one
