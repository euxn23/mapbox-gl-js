...
import { registerForPluginAvailability, evented as rtlTextPluginEvented } from '../source/rtl_text_plugin';
import PauseablePlacement from './pauseable_placement';
import ZoomHistory from './zoom_history';
import CrossTileSymbolIndex from '../symbol/cross_tile_symbol_index'; // We're skipping validation errors with the `source.canvas` identifier in order
...
const supportedDiffOperations = pick(diffOperations, ['addLayer', 'removeLayer', 'setPaintProperty', 'setLayoutProperty', 'setFilter', 'addSource', 'removeSource', 'setLayerZoomRange', 'setLight', 'setTransition', 'setGeoJSONSourceData' // 'setGlyphs',
// 'setSprite',
]);
...
class Style extends Evented {
  ...
  _validateLayer(layer) {
    ...
    if (source.type === 'geojson' || source.vectorLayerIds && source.vectorLayerIds.indexOf(sourceLayer) === -1) {
      this.fire(new ErrorEvent(new Error(`Source layer "${sourceLayer}" ` + `does not exist on source "${source.id}" ` + `as specified by style layer "${layer.id}"`)));
    }
  }
  ...
  */
  ...
  addLayer(layerObject, before, options) {
    ...
    if (this._validate(validateStyle.layer, `layers.${id}`, layerObject, {
      arrayIndex: -1
    }, options)) return;
  ...
  queryRenderedFeatures(queryGeometry, params, transform) {
    ...
    for (const id in this.sourceCaches) {
      ...
      sourceResults.push(queryRenderedFeatures(this.sourceCaches[id], this._layers, queryGeometry.worldCoordinate, params, transform));
    }

    if (this.placement) {
      // If a placement has run, query against its CollisionIndex
      // for symbol results, and treat it as an extra source to merge
      sourceResults.push(queryRenderedSymbols(this._layers, queryGeometry.viewport, params, this.placement.collisionIndex, this.placement.retainedQueryData));
    }
  ...
  _updatePlacement(transform, showCollisionBoxes, fadeDuration) {
    ...
    for (const layerID of this._order) {
      ...
      if (!layerTiles[styleLayer.source]) {
        ...
        layerTiles[styleLayer.source] = sourceCache.getRenderableIds().map(id => sourceCache.getTileByID(id)).sort((a, b) => b.tileID.overscaledZ - a.tileID.overscaledZ || (a.tileID.isLessThan(b.tileID) ? -1 : 1));
      }
    ...
    this.crossTileSymbolIndex.pruneUnusedLayers(this._order); // Anything that changes our "in progress" layer and tile indices requires us
