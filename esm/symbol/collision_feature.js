...
class CollisionFeature {
   ...
   */
  constructor(collisionBoxArray, line, anchor, featureIndex, sourceLayerIndex, bucketIndex, shaped, boxScale, padding, alignLine, overscaling) {
  ...
  _addLineCollisionCircles(collisionBoxArray, line, anchor, segment, labelLength, boxSize, featureIndex, sourceLayerIndex, bucketIndex, overscaling) {
    ...
    const nBoxes = Math.floor(labelLength / step); // We calculate line collision circles out to 300% of what would normally be our
    ...
    const nPitchPaddingBoxes = Math.floor(nBoxes * overscalingPaddingFactor / 2); // offset the center of the first box by half a box so that the edge of the
    ...
    const paddingStartDistance = labelStartDistance - labelLength / 4; // move backwards along the line to the first segment the label appears on
    ...
    for (let i = -nPitchPaddingBoxes; i < nBoxes + nPitchPaddingBoxes; i++) {
      ...
      let boxDistanceToAnchor = labelStartDistance + boxOffset; // make the distance between pitch padding boxes bigger
      ...
      while (anchorDistance + segmentLength < boxDistanceToAnchor) {
        ...
        index++; // There isn't enough room before the end of the line.
      ...
      const boxAnchorPoint = p1.sub(p0)._unit()._mult(segmentBoxDistance)._add(p0)._round(); // If the box is within boxSize of the anchor, force the box to be used
