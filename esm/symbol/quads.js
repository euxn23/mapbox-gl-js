 ...
 * @private
 */
export function getIconQuads(anchor, shapedIcon, layer, alongLine, shapedText, globalProperties, feature) {
  const image = shapedIcon.image;
  const layout = layer.layout; // If you have a 10px icon that isn't perfectly aligned to the pixel grid it will cover 11 actual
  ...
  if (layout.get('icon-text-fit') !== 'none' && shapedText) {
    ...
    bl = new Point(textLeft + offsetX - padL, textTop + offsetY + padB + height); // Normal icon size mode
  } else {
...
export function getGlyphQuads(anchor, shaping, layer, alongLine, globalProperties, feature, positions) {
  const oneEm = 24;
  ...
  for (let k = 0; k < positionedGlyphs.length; k++) {
    ...
    if (!rect) continue; // The rects have an addditional buffer that is not included in their size.
