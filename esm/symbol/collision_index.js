...
class CollisionIndex {
  ...
  placeCollisionCircles(collisionCircles, allowOverlap, scale, textPixelRatio, key, symbol, lineVertexArray, glyphOffsetArray, fontSize, posMatrix, labelPlaneMatrix, showCollisionCircles, pitchWithMap) {
    ...
    const tileUnitAnchorPoint = new Point(symbol.anchorX, symbol.anchorY); // projection.project generates NDC coordinates, as opposed to the
    ...
    const firstAndLastGlyph = projection.placeFirstAndLastGlyph(fontScale, glyphOffsetArray, lineOffsetX, lineOffsetY,
    /*flip*/
    false, labelPlaneAnchorPoint, tileUnitAnchorPoint, symbol, lineVertexArray, labelPlaneMatrix, projectionCache,
    /*return tile distance*/
    true);
    let collisionDetected = false;
    let entirelyOffscreen = true;
    const tileToViewport = projectedAnchor.perspectiveRatio * textPixelRatio; // pixelsToTileUnits is used for translating line geometry to tile units
    ...
    for (let k = 0; k < collisionCircles.length; k += 5) {
      ...
      if (atLeastOneCirclePlaced) {
        ...
        const dy = projectedPoint.y - placedCollisionCircles[placedCollisionCircles.length - 3]; // The circle edges touch when the distance between their centers is 2x the radius
  ...
  queryRenderedSymbols(viewportQueryGeometry) {
    ...
    for (const feature of features) {
      const featureKey = feature.key; // Skip already seen features.
      ...
      const bbox = [new Point(feature.x1, feature.y1), new Point(feature.x2, feature.y1), new Point(feature.x2, feature.y2), new Point(feature.x1, feature.y2)];
  ...
  projectPoint(posMatrix, x, y) {
    const p = [x, y, 0, 1];
    projection.xyTransformMat4(p, p, posMatrix);
    return new Point((p[0] / p[3] + 1) / 2 * this.transform.width + viewportPadding, (-p[1] / p[3] + 1) / 2 * this.transform.height + viewportPadding);
  }
