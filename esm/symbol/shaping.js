import { charHasUprightVerticalOrientation, charAllowsIdeographicBreaking } from '../util/script_detection';
...
export { shapeText, shapeIcon, WritingMode }; // The position of a glyph relative to the text's anchor point.
...
function calculatePenalty(codePoint, nextCodePoint) {
  let penalty = 0; // Force break on newline
...
function determineLineBreaks(logicalInput, spacing, maxWidth, glyphs) {
  ...
  for (let i = 0; i < logicalInput.length; i++) {
    ...
    if (glyph && !whitespace[codePoint]) currentX += glyph.metrics.advance + spacing; // Ideographic characters, spaces, and word-breaking punctuation that often appear without
    // surrounding spaces.
...
function shapeLines(shaping, glyphs, lines, lineHeight, textAnchor, textJustify, writingMode, spacing, verticalHeight) {
  ...
  align(positionedGlyphs, justify, horizontalAlign, verticalAlign, maxLineLength, lineHeight, lines.length); // Calculate the bounding box
