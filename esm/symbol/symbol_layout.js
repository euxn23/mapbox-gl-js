...
import Point from '@mapbox/point-geometry'; // The symbol layout process needs `text-size` evaluated at up to five different zoom levels, and
...
export function performSymbolLayout(bucket, glyphMap, glyphPositions, imageMap, imagePositions, showCollisionBoxes) {
  ...
  if (bucket.textSizeData.functionType === 'composite') {
    ...
    sizes.compositeTextSizes = [unevaluatedLayoutValues['text-size'].possiblyEvaluate(new EvaluationParameters(min)), unevaluatedLayoutValues['text-size'].possiblyEvaluate(new EvaluationParameters(max))];
  }
  ...
  if (bucket.iconSizeData.functionType === 'composite') {
    ...
    sizes.compositeIconSizes = [unevaluatedLayoutValues['icon-size'].possiblyEvaluate(new EvaluationParameters(min)), unevaluatedLayoutValues['icon-size'].possiblyEvaluate(new EvaluationParameters(max))];
  }
  ...
  for (const feature of bucket.features) {
    ...
    if (text) {
      ...
      const maxWidth = layout.get('symbol-placement') !== 'line' ? layout.get('text-max-width').evaluate(feature) * oneEm : 0;
...
function addFeature(bucket, feature, shapedTextOrientations, shapedIcon, glyphPositionMap, sizes) {
  ...
  const layoutIconSize = sizes.layoutIconSize.evaluate(feature); // To reduce the number of labels that jump around when zooming we need
  ...
  const addSymbolAtAnchor = (line, anchor) => {
    ...
    bucket.symbolInstances.push(addSymbol(bucket, anchor, line, shapedTextOrientations, shapedIcon, bucket.layers[0], bucket.collisionBoxArray, feature.index, feature.sourceLayerIndex, bucket.index, textBoxScale, textPadding, textAlongLine, textOffset, iconBoxScale, iconPadding, iconAlongLine, iconOffset, {
      zoom: bucket.zoom
  ...
  if (symbolPlacement === 'line') {
    for (const line of clipLine(feature.geometry, 0, 0, EXTENT, EXTENT)) {
      const anchors = getAnchors(line, symbolMinDistance, textMaxAngle, shapedTextOrientations.vertical || shapedTextOrientations.horizontal, shapedIcon, glyphSize, textMaxBoxScale, bucket.overscaling, EXTENT);
...
function addTextVertices(bucket, anchor, shapedText, layer, textAlongLine, globalProperties, feature, textOffset, lineArray, writingMode, placedTextSymbolIndices, glyphPositionMap, sizes) {
  const glyphQuads = getGlyphQuads(anchor, shapedText, layer, textAlongLine, globalProperties, feature, glyphPositionMap);
  ...
  if (sizeData.functionType === 'source') {
    ...
    textSizeData = [10 * sizes.compositeTextSizes[0].evaluate(feature), 10 * sizes.compositeTextSizes[1].evaluate(feature)];
  }

  bucket.addSymbols(bucket.text, glyphQuads, textSizeData, textOffset, textAlongLine, feature, writingMode, anchor, lineArray.lineStartIndex, lineArray.lineLength); // The placedSymbolArray is used at render time in drawTileSymbols
...
function addSymbol(bucket, anchor, line, shapedTextOrientations, shapedIcon, layer, collisionBoxArray, featureIndex, sourceLayerIndex, bucketIndex, textBoxScale, textPadding, textAlongLine, textOffset, iconBoxScale, iconPadding, iconAlongLine, iconOffset, globalProperties, feature, glyphPositionMap, sizes) {
  ...
  if (shapedIcon) {
    const iconQuads = getIconQuads(anchor, shapedIcon, layer, iconAlongLine, shapedTextOrientations.horizontal, globalProperties, feature);
    iconCollisionFeature = new CollisionFeature(collisionBoxArray, line, anchor, featureIndex, sourceLayerIndex, bucketIndex, shapedIcon, iconBoxScale, iconPadding,
    /*align boxes to line*/
    false, bucket.overscaling);
    ...
    if (sizeData.functionType === 'source') {
      ...
      iconSizeData = [10 * sizes.compositeIconSizes[0].evaluate(feature), 10 * sizes.compositeIconSizes[1].evaluate(feature)];
    }
