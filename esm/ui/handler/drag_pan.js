...
class DragPanHandler {
  ...
  constructor(map) {
    ...
    bindAll(['_onMove', '_onMouseUp', '_onTouchEnd', '_onBlur', '_onDragFrame'], this);
  }
  /**
  ...
  onMouseDown(e) {
    ...
    if (e.ctrlKey || DOM.mouseButton(e) !== 0) return; // Bind window-level event listeners for mousemove/up events. In the absence of
  ...
  onTouchStart(e) {
    if (this._state !== 'enabled') return;
    if (e.touches.length > 1) return; // Bind window-level event listeners for touchmove/end events. In the absence of
    // the pointer capture API, which is not supported by all necessary platforms,
    // window-level event listeners give us the best shot at capturing events that
    // fall outside the map canvas element. Use `{capture: true}` for the move event
    // to prevent map move events from being fired during a drag.
