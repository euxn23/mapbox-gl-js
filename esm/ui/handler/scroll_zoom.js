...
const wheelZoomDelta = 4.000244140625; // These magic numbers control the rate of zoom. Trackpad events fire at a greater
// frequency than mouse scroll wheel, so reduce the zoom rate per wheel tick

const defaultZoomRate = 1 / 100;
const wheelZoomRate = 1 / 450; // upper bound on how much we scale the map in any single render frame; this
...
class ScrollZoomHandler {
  ...
  constructor(map) {
    ...
    this._delta = 0;
    bindAll(['_onWheel', '_onTimeout', '_onScrollFrame', '_onScrollFinished'], this);
  }
  /**
  ...
  onWheel(e) {
    if (!this.isEnabled()) return; // Remove `any` cast when https://github.com/facebook/flow/issues/4879 is fixed.
    ...
    if (value !== 0 && value % wheelZoomDelta === 0) {
      ...
      this._type = null;
      this._lastValue = value; // Start a timeout in case this was a singular event, and dely it by up to 40ms.

      this._timeout = setTimeout(this._onTimeout, 40, e);
    } else if (!this._type) {
      // This is a repeating event, but we don't know the type of event just yet.
      // If the delta per time is small, we assume it's a fast trackpad; otherwise we switch into wheel mode.
      this._type = Math.abs(timeDelta * value) < 200 ? 'trackpad' : 'wheel'; // Make sure our delayed event isn't fired again, because we accumulate
    ...
    if (e.shiftKey && value) value = value / 4; // Only fire the callback if we actually know what type of scrolling device the user uses.
  ...
  _onScrollFrame() {
    ...
    const tr = this._map.transform; // if we've had scroll events since the last render frame, consume the
    // accumulated delta, and update the target zoom level accordingly

    if (this._delta !== 0) {
      // For trackpad events and single mouse wheel ticks, use the default zoom rate
      const zoomRate = this._type === 'wheel' && Math.abs(this._delta) > wheelZoomDelta ? wheelZoomRate : defaultZoomRate; // Scale by sigmoid of scroll wheel delta.
      ...
      this._targetZoom = Math.min(tr.maxZoom, Math.max(tr.minZoom, tr.scaleZoom(fromScale * scale))); // if this is a mouse wheel, refresh the starting zoom and easing
