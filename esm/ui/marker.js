...
export default class Marker {
  constructor(options) {
    ...
    if (!options || !options.element) {
      ...
      this._element = DOM.create('div'); // create default map marker SVG
      ...
      this._element.appendChild(svg); // if no element and no offset option given apply an offset for the default marker
  ...
  addTo(map) {
    ...
    this._update(); // If we attached the `click` listener to the marker element, the popup
  ...
  togglePopup() {
    ...
    if (!popup) return this;else if (popup.isOpen()) popup.remove();else popup.addTo(this._map);
    return this;
  }
  ...
  _update(e) {
    ...
    this._pos = this._map.project(this._lngLat)._add(this._offset); // because rounding the coordinates at every `move` event causes stuttered zooming
