...
class GeolocateControl extends Evented {
  ...
  constructor(options) {
    ...
    bindAll(['_onSuccess', '_onError', '_finish', '_setupUI', '_updateCamera', '_updateMarker'], this);
  }
      ...
      if (error.code === 1) {
        ...
        switch (this._watchState) {
          ...
          case 'ACTIVE_LOCK':
            ...
            this._geolocateButton.classList.add('mapboxgl-ctrl-geolocate-waiting'); // turn marker grey
  ...
  _setupUI(supported) {
    ...
    this._setup = true; // when the camera is changed (and it's not as a result of the Geolocation Control) change
    ...
    if (this.options.trackUserLocation) {
      ...
      switch (this._watchState) {
        ...
        case 'BACKGROUND':
          ...
          this._geolocateButton.classList.remove('mapboxgl-ctrl-geolocate-background'); // set camera to last known location
      ...
      window.navigator.geolocation.getCurrentPosition(this._onSuccess, this._onError, this.options.positionOptions); // This timeout ensures that we still call finish() even if
