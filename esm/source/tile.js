...
class Tile {
  ...
  constructor(tileID, size) {
    ...
    this.queryPadding = 0; // Counts the number of times a response was already expired when
  ...
  loadVectorData(data, painter, justReloaded) {
    ...
    this.state = 'loaded'; // empty GeoJSON tile
  ...
  queryRenderedFeatures(layers, queryGeometry, scale, params, transform, maxPitchScaleFactor, posMatrix) {
  ...
  querySourceFeatures(result, params) {
    ...
    if (!this.latestFeatureIndex.vtLayers) {
      this.latestFeatureIndex.vtLayers = new vt.VectorTile(new Protobuf(this.latestFeatureIndex.rawTileData)).layers;
    }
  ...
  setMask(mask, context) {
    ...
    this.mask = mask;
    this.clearMask(); // We want to render the full tile, and keeping the segments/vertices/indices empty means
    ...
    this.segments = new SegmentVector(); // Create a new segment so that we will upload (empty) buffers even when there is nothing to
    // draw for this tile.
    ...
    for (let i = 0; i < maskArray.length; i++) {
      ...
      const brVertex = new Point(tlVertex.x + vertexExtent, tlVertex.y + vertexExtent); // not sure why flow is complaining here because it doesn't complain at L401
      ...
      const offset = segment.vertexLength; // 0, 1, 2
      // 1, 2, 3
