...
function loadGeoJSONTile(params, callback) {
  ...
  const geojsonWrapper = new GeoJSONWrapper(geoJSONTile.features); // Encode the geojson-vt tile into binary vector tile form form.  This
  ...
  _loadData() {
    ...
    this.loadGeoJSON(params, (err, data) => {
      if (err || !data) {
        ...
        try {
          this._geoJSONIndex = params.cluster ? supercluster(params.superclusterOptions).load(data.features) : geojsonvt(data, params.geojsonVtOptions);
        } catch (err) {
          return callback(err);
        }
        ...
        if (params.request && params.request.collectResourceTiming) {
          const resourceTimingData = perf.getEntriesByName(params.request.url); // it's necessary to eval the result of getEntriesByName() here via parse/stringify
