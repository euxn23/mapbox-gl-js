...
class SourceCache extends Evented {
  constructor(id, options, dispatcher) {
    ...
    this.on('data', e => {
      ...
      if (e.dataType === 'source' && e.sourceDataType === 'metadata') this._sourceLoaded = true; // for sources with mutable data, this event fires when the underlying data
  ...
  _reloadTile(id, state) {
    const tile = this._tiles[id]; // this potentially does not address all underlying
    // issues https://github.com/mapbox/mapbox-gl-js/issues/4252
    // - hard to tell without repro steps

    if (!tile) return; // The difference between "loading" tiles and "reloading" or "expired"
  ...
  _tileLoaded(tile, id, previousState, err) {
    if (err) {
      tile.state = 'errored';
      if (err.status !== 404) this._source.fire(new ErrorEvent(err, {
        tile
  ...
  */
  ...
  _findLoadedChildren(tileID, maxCoveringZoom, retain) {
    ...
    for (const id in this._tiles) {
      let tile = this._tiles[id]; // only consider renderable tiles on higher zoom levels (up to maxCoveringZoom)

      if (retain[id] || !tile.hasData() || tile.tileID.overscaledZ <= tileID.overscaledZ || tile.tileID.overscaledZ > maxCoveringZoom) continue; // disregard tiles that are not descendants of the given tile coordinate

      const z2 = Math.pow(2, tile.tileID.canonical.z - tileID.canonical.z);
      if (Math.floor(tile.tileID.canonical.x / z2) !== tileID.canonical.x || Math.floor(tile.tileID.canonical.y / z2) !== tileID.canonical.y) continue; // found loaded child

      retain[id] = tile.tileID;
      found = true; // loop through parents; retain the topmost loaded one if found
  ...
  update(transform) {
    ...
    this.updateCacheSize(transform); // Covered is a list of retained tiles who's areas are fully covered by other,
    ...
    const maxCoveringZoom = Math.max(zoom + SourceCache.maxUnderzooming, this._source.minzoom); // Retain is a list of tiles that we shouldn't delete, even if they are not
    ...
    if (isRasterType(this._source.type)) {
      ...
      for (let k = 0; k < ids.length; k++) {
        ...
        const tile = this._tiles[id];
        if (!tile) continue; // If the drawRasterTile has never seen this tile, then
  ...
  _updateRetainedTiles(idealTileIDs, zoom) {
    ...
    for (let i = 0; i < idealTileIDs.length; i++) {
      ...
      if (tile.hasData()) {
        ...
        parentWasRequested = tile.wasRequested(); // The tile isn't loaded yet, but retain it anyway because it's an ideal tile.
        ...
        if (overscaledZ > this._source.maxzoom) {
          ...
          this._findLoadedChildren(tileID, maxCoveringZoom, retain); // check if all 4 immediate children are loaded (i.e. the missing ideal tile is covered)
        ...
        if (!covered) {
          ...
          for (let overscaledZ = tileID.overscaledZ - 1; overscaledZ >= minCoveringZoom; --overscaledZ) {
            ...
            if (tile) {
              retain[parentId.key] = parentId; // Save the current values, since they're the parent of the next iteration
  ...
  _resetCache() {
    for (const id in this._cacheTimers) clearTimeout(this._cacheTimers[id]);
  ...
  tilesIn(queryGeometry, maxPitchScaleFactor) {
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      const tileSpaceBounds = [coordinateToTilePoint(tileID, new Coordinate(minX, minY, z)), coordinateToTilePoint(tileID, new Coordinate(maxX, maxY, z))];
...
function coordinateToTilePoint(tileID, coord) {
  ...
  return new Point((zoomedCoord.column - (tileID.canonical.x + tileID.wrap * Math.pow(2, tileID.canonical.z))) * EXTENT, (zoomedCoord.row - tileID.canonical.y) * EXTENT);
}
