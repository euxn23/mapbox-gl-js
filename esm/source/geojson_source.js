...
class GeoJSONSource extends Evented {
  ...
  constructor(id, options, dispatcher, eventedParent) {
    super();
    this.id = id; // `type` is a property rather than a constant to make it easy for 3rd
    ...
    const scale = EXTENT / this.tileSize; // sent to the worker, along with `url: ...` or `data: literal geojson`,
  ...
  _updateWorkerData(callback) {
    ...
    this.workerID = this.dispatcher.send(`${this.type}.${options.source}.loadData`, options, (err, result) => {
      ...
      this._loaded = true;
      if (result && result.resourceTiming && result.resourceTiming[this.id]) this._resourceTiming = result.resourceTiming[this.id].slice(0); // Any `loadData` calls that piled up while we were processing
