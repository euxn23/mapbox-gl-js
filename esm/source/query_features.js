...
export function queryRenderedFeatures(sourceCache, styleLayers, queryGeometry, params, transform) {
  ...
  for (const tileIn of tilesIn) {
    renderedFeatureLayers.push({
      ...
      queryResults: tileIn.tile.queryRenderedFeatures(styleLayers, tileIn.queryGeometry, tileIn.scale, params, transform, maxPitchScaleFactor, sourceCache.transform.calculatePosMatrix(tileIn.tileID.toUnwrapped()))
    });
  }

  return mergeRenderedFeatureLayers(renderedFeatureLayers);
}
export function queryRenderedSymbols(styleLayers, queryGeometry, params, collisionIndex, retainedQueryData) {
  const result = {};
  ...
  for (const queryData of bucketQueryData) {
    const bucketSymbols = queryData.featureIndex.lookupSymbolFeatures(renderedSymbols[queryData.bucketInstanceId], queryData.bucketIndex, queryData.sourceLayerIndex, params.filter, params.layers, styleLayers);
