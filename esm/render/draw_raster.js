...
function drawRaster(painter, sourceCache, layer, coords) {
  ...
  context.setColorMode(painter.colorModeForRenderPass()); // Constant parameters.
  ...
  for (const coord of coords) {
    ...
    context.setDepthMode(painter.depthModeForSublayer(coord.overscaledZ - minTileZ, layer.paint.get('raster-opacity') === 1 ? DepthMode.ReadWrite : DepthMode.ReadOnly, gl.LESS));
...
function getFadeValues(tile, parentTile, sourceCache, layer, transform) {
  ...
  if (fadeDuration > 0) {
    ...
    const childOpacity = fadeIn && tile.refreshedUponExpiration ? 1 : clamp(fadeIn ? sinceTile : 1 - sinceParent, 0, 1); // we don't crossfade tiles that were just refreshed upon expiring:
