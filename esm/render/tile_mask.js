...
export default function (renderableTiles, context) {
  ...
  for (let i = 0; i < sortedRenderables.length; i++) {
    ...
    const childArray = sortedRenderables.slice(i + 1); // Try to add all remaining ids as children. We sorted the tile list
...
function computeTileMasks(rootTile, ref, childArray, lowerBound, mask) {
  ...
  for (let i = 0; i < childArray.length; i++) {
    const childTile = childArray[i]; // childTile is from a larger wrap than the rootTile so it cannot be a child tile

    if (lowerBound.isLessThan(childTile.tileID)) break; // The current tile is masked out, so we don't need to add them to the mask set.
