...
class Painter {
  constructor(gl, transform) {
    ...
    this._tileTextures = {};
    this.setup(); // Within each layer there are multiple distinct z-planes that can be drawn to.
  ...
  clearStencil() {
    const context = this.context;
    const gl = context.gl; // As a temporary workaround for https://github.com/mapbox/mapbox-gl-js/issues/5490,
    ...
    context.setStencilMode(new StencilMode({
      func: gl.ALWAYS,
      mask: 0
  ...
  _renderTileClippingMasks(tileIDs) {
    ...
    for (const tileID of tileIDs) {
      const id = this._tileClippingMaskIDs[tileID.key] = idNext++; // Tests will always pass, and ref value will be written to stencil buffer.
      ...
      const program = this.useProgram('clippingMask');
      gl.uniformMatrix4fv(program.uniforms.u_matrix, false, tileID.posMatrix); // Draw the clipping mask
  ...
  render(style, options) {
    ...
    const rasterSources = filterObject(this.style.sourceCaches, sc => {
      return sc.getSource().type === 'raster' || sc.getSource().type === 'raster-dem';
    });

    for (const key in rasterSources) {
      const sourceCache = rasterSources[key];
      const coords = sourceCache.getVisibleCoordinates();
      const visibleTiles = coords.map(c => {
        return sourceCache.getTile(c);
      });
    ...
    this.depthRange = (style._order.length + 2) * this.numSublayers * this.depthEpsilon; // Opaque pass
  ...
  setupOffscreenDepthRenderbuffer() {
    const context = this.context; // All of the 3D textures will use the same depth renderbuffer.
  ...
  translatePosMatrix(matrix, tile, translate, translateAnchor, inViewportPixelUnitsUnits) {
    ...
    const translation = [inViewportPixelUnitsUnits ? translate[0] : pixelsToTileUnits(tile, translate[0], this.transform.zoom), inViewportPixelUnitsUnits ? translate[1] : pixelsToTileUnits(tile, translate[1], this.transform.zoom), 0];
