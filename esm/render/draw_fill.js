import { isPatternMissing, setPatternUniforms, prepare as preparePattern } from './pattern';
...
function drawFill(painter, sourceCache, layer, coords) {
  ...
  const pass = !layer.paint.get('fill-pattern') && color.constantOr(Color.transparent).a === 1 && opacity.constantOr(0) === 1 ? 'opaque' : 'translucent'; // Draw fill
  ...
  if (painter.renderPass === 'translucent' && layer.paint.get('fill-antialias')) {
    context.lineWidth.set(2); // If we defined a different color for the fill outline, we are
    ...
    context.setDepthMode(painter.depthModeForSublayer(layer.getPaintProperty('fill-outline-color') ? 2 : 0, DepthMode.ReadOnly));
...
function drawFillTile(painter, sourceCache, layer, tile, coord, bucket, firstTile) {
  ...
  program.draw(painter.context, gl.TRIANGLES, layer.id, bucket.layoutVertexBuffer, bucket.indexBuffer, bucket.segments, programConfiguration);
}
...
function drawStrokeTile(painter, sourceCache, layer, tile, coord, bucket, firstTile) {
  ...
  program.draw(painter.context, gl.LINES, layer.id, bucket.layoutVertexBuffer, bucket.indexBuffer2, bucket.segments2, programConfiguration);
}
...
function setFillProgram(programId, pat, painter, programConfiguration, layer, tile, coord, firstTile) {
  ...
  painter.context.gl.uniformMatrix4fv(program.uniforms.u_matrix, false, painter.translatePosMatrix(coord.posMatrix, tile, layer.paint.get('fill-translate'), layer.paint.get('fill-translate-anchor')));
  return program;
}