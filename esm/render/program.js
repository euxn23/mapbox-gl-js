...
class Program {
  constructor(context, source, configuration, showOverdrawInspector) {
    const gl = context.gl;
    ...
    gl.attachShader(this.program, vertexShader); // Manually bind layout attributes in the order defined by their
  ...
  draw(context, drawMode, layerID, layoutVertexBuffer, indexBuffer, segments, configuration, dynamicLayoutBuffer, dynamicLayoutBuffer2) {
    const gl = context.gl;
    const primitiveSize = {
      [gl.LINES]: 2,
      [gl.TRIANGLES]: 3
